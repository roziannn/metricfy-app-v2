...
export function AdminCourseCard({ data }: iAppProps) {
  ...
  return (
    <Card className="group relative py-0 gap-0">
      ...
      <Image
        src={thumbnailUrl}
        alt="Thumbnail url pic"
        width={600}
        height={400}
        className="w-full rounded-t-lg aspect-video h-full object-cover"
      />
      {/* kalo pake img compt di next js dan pake remote url, hrs di define width dan height property.
        Kalo local gaperlu define/invert width and hight */}
      <CardContent className="p-4">
        ...
        <Link href={`/admin/courses/${data.id}/edit`} className={buttonVariants({ className: "w-full mt-4" })}>
          Edit Course <ArrowRight className="size-4"></ArrowRight>
        </Link>
      </CardContent>
    </Card>
  );
